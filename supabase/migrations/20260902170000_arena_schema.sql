-- =====================================================================
-- 20260902170000_arena_schema.sql — 非同期PvPアリーナのテーブル定義
--
-- ## 名前の約束
--
...
--
-- ## 順番
--   20260902170000_arena_schema.sql      … 表・索引・制約(このファイル)
--   20260902170100_arena_rls.sql  … RLS と権限
--   20260902172000_arena_rpc.sql  … RPC(レート計算・報酬・購入)
-- =====================================================================
...
insert into public.arena_config (key, value, note) values
  ...
  ('tickets',
   '{"max":10,"refill_minutes":60,"cost_per_match":1}'::jsonb,
   '挑戦券。回復はサーバ時刻で計算する。ARENA_TICKET_MAX / ARENA_TICKET_REGEN_MINUTES と同じ値にすること'),
  ('match_coins',
   '{"win_base":10,"loss_base":3,"upset_step":50,"upset_max":0}'::jsonb,
   '1戦で入るアリーナコイン。ARENA_COIN_WIN / ARENA_COIN_LOSS と同じ値にすること。'
   || '格上ボーナス(upset_max)はクライアント側に無いので0。片方だけ増やすと、繋いだ瞬間に数字が飛ぶ'),
  ('npc',
