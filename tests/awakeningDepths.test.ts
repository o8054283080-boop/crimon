...
describe("階の中身", () => {
  ...
  it("ボスの実効値は依頼の表どおり。**倍率ではなく実数で置く**", () => {
    const expected = [
      ...
      [125_000, 6_200, 3_000, 164], [145_000, 7_200, 3_400, 173],
      /*
       * 9階・10階は9/7に下げた。
       *
       * **お供を倒すとボスが強くなる**仕掛けが、画面から遊ぶ道では
       * 呼ばれていなかった(`BattleEngine.recordTurn` へ移して直した)。
       * 効くようにした結果、9階が45%・10階が5%まで落ちたため、
       * 依頼主の判断で両方を緩めてある。
       */
      [145_000, 7_100, 3_700, 180],
      [145_000, 7_000, 4_000, 185],
    ];
