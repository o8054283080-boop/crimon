...
describe("試練の塔 51〜99階: 上へ行くほど重い(実際に戦わせて測る)", () => {
  ...
  it("59→69→79階と、上の帯ほど決着までの手数が伸びる", () => {
     ...
     *
     * 実測(20回平均): 11.9 → 15.1 → 19.9 → 18.9 → 53.2。
     *
     * **89階は79階とほぼ同じ重さ。**以前はここが 24.15 対 23.45 で
     * かろうじて上回っていたが、それは**時間切れ(300手)が混ざっていたから**で、
     * 素の重さではない。ドラゴンのS2を多段へ変えて勝率が100%になり
     * 時間切れが消えると、79階と89階が並んだ。
     *
     * 81〜89階の帯は71〜79階に対して**実数で1.2倍しか伸ばしていない**ので、
     * 手数がほぼ横並びなのは作りのとおり。だから89階は「79階より重い」ではなく
     * **「79階から落ち込まない」**を見る。伸びの本体は99階で見張る。
     */
    ...
    expect(turnsToSettle(89), "89階の手数").toBeGreaterThan(lower[2] * 0.9);
...
describe("今回触っていないところ", () => {
  ...
  it("1〜50階の敵編成と難易度が変わっていない", () => {
    // 2026-10: 攻撃役の草ナイトを火ナイトへ(「たてうけ」に全体シールドが付いたため。
    // 癒やし・守り以外の階に盾持ちを置かない規則を守る)。到達階の中央値は変わらない
    expect(digest(1, 50)).toBe("576ad5ff7cabb186");
  });

  it("80階は古代聖竜とお供4体の専用編成", () => {
    const def = findTowerFloor(80)!;
    expect(def.enemies).toHaveLength(5);
    expect(def.enemies[0].displayName).toBe("古代聖竜");
    for (const enemy of def.enemies) expect(enemy.fixedStats).toBeDefined();
  });
