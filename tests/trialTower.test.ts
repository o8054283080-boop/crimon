...
describe("試練の塔: 難易度が上へ向かって単調に重くなる", () => {
  it("同じ編成が全回復から挑んだとき、上の階ほど残りHPが減る", () => {
     ...
     * **試行数はここだけ増やしてある。**
     *
     * 10階と20階はどちらも味方が9割以上残して勝つ軽い階で、その差は1pt未満。
     * 既定の6回だと乱数のぶれが差を飲み込み、順序が入れ替わる
     * (実際に 0.9305 対 0.9311 で逆転して落ちた)。24回まで増やすと安定する。
     * **落ちた時に真っ先に疑うのは、ここの試行数ではなく階の中身の方。**
     */
    const costs = [10, 20, 30].map((floor) => floorCost(TEAMS.通常, floor, 24));
...
describe("試練の塔: 到達階(登り切るまで実際に登らせて測る)", () => {
   ...
   * **中央値ではなく「いちばん悪い回」で見る。**
   *
   * 耐久編成の取り柄は、深さそのものではなく**ばらつかないこと**だと
   * 実測で分かった(30回登らせて到達階が全部同じ)。通常編成は運がよければ
   * もっと上まで行くが、悪いと数階下で終わる。
   *
   * 深さ自体で並べられなくなったのは、**26階が火力を要求する階**だから。
   * 装備なしで10回ずつ測ると、耐久も通常も26階は300手の時間切れになる
   * (耐久 0% / 通常 0%)。硬さを1.5〜2倍にしても越えられない壁で、
   * 役割ごとの底上げを入れても耐久編成は25階のまま動かなかった。
   * 動いたのは通常編成の方(23→26階)で、そこにヒーラーとデバッファーが
   * 入っているぶん底上げを多く受けたため。
   */
  /*
   * **「いちばん悪い回」では不安定だった。**耐久編成は30回中28回が同じ階で
   * 止まる代わりに、残り2回は事故って数階下で終わる。試行12回のここでは
   * その2回を引くかどうかで結果が変わり、乱数の引きを測るテストになっていた。
   * 中央値どうしを1階の幅で比べる。
   */
  it("持ち越しの塔なので、耐久編成は通常編成に並ぶところまで行ける", () => {
