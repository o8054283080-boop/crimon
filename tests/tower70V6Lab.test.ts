...
import { buildTower70V6, TOWER70_HEAL_BLOCK_3, TOWER70_POISON_3 } from "../tools/battleLab/scenarios/tower70v6.js";
...
describe("70階V6: 攻撃型パッシブ + 回復阻害3体/毒3体", () => {
  ...
  it("V6 回復阻害3体 1000戦×5攻略順", () => {
    ...
    const rows = TOWER70_FOCUS.map((pattern) => {
      const tallies = runMany(scenario, 20260906, 1000, pattern.order, "TYPICAL");
    ...
    console.log("TOWER70_V6_HEAL_BLOCK3_RESULTS=" + JSON.stringify(rows));
    expect(rows).toHaveLength(5);
  }, 180_000);

  it("V6 毒3体 1000戦×5攻略順", () => {
    const scenario = buildTower70V6(TOWER70_POISON_3);
    const rows = TOWER70_FOCUS.map((pattern) => {
      const tallies = runMany(scenario, 20260907, 1000, pattern.order, "TYPICAL");
      const wins = tallies.filter((t) => t.winner === "PLAYER").length;
      const losses = tallies.filter((t) => t.winner === "ENEMY").length;
      const draws = tallies.filter((t) => t.winner === "DRAW").length;
      const extraMean = (key: string) => mean(tallies.map((t) => t.extra[key] ?? 0));
    ...
    console.log("TOWER70_V6_POISON3_RESULTS=" + JSON.stringify(rows));
    expect(rows).toHaveLength(5);
  }, 180_000);
});
