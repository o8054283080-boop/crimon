...
import { TOWER90_ENEMIES_V6, TOWER90_RUSH_V6, TOWER90_SAFE_V6 } from "../tools/battleLab/scenarios/tower90v6.js";

const mean = (xs: number[]) => xs.length ? xs.reduce((a, b) => a + b, 0) / xs.length : 0;

describe("90階V6 6000戦の一時測定", () => {
  it("V6ステータスを固定する", () => {
    expect(TOWER90_ENEMIES_V6.map((e) => [e.stats?.hp, e.stats?.atk, e.stats?.def, e.stats?.spd, e.stats?.accuracy, e.stats?.resistance])).toEqual([
  ...
  it("安全処理3順+速攻3順を各1000戦", () => {
    const rows = [
      ...TOWER90_SAFE_FOCUS.map((focus, i) => ({ type: "SAFE", focus, scenario: TOWER90_SAFE_V6, seed: 20311190 + i * 10_000 })),
      ...TOWER90_RUSH_FOCUS.map((focus, i) => ({ type: "RUSH", focus, scenario: TOWER90_RUSH_V6, seed: 20321190 + i * 10_000 })),
    ...
    console.log("TOWER90_V6_RESULTS=" + JSON.stringify(rows));
