...
import { TOWER90_ENEMIES_V7, TOWER90_RUSH_V7, TOWER90_SAFE_V7 } from "../tools/battleLab/scenarios/tower90v7.js";

const mean = (xs: number[]) => xs.length ? xs.reduce((a, b) => a + b, 0) / xs.length : 0;

describe("90階V7 6000戦の一時測定", () => {
  it("V7ステータスを固定する", () => {
    expect(TOWER90_ENEMIES_V7.map((e) => [e.stats?.hp, e.stats?.atk, e.stats?.def, e.stats?.spd, e.stats?.accuracy, e.stats?.resistance])).toEqual([
      [350_000, 9_000, 4_200, 200, 0.65, 0.50],
      [210_000, 7_000, 3_200, 175, 0.65, 0.50],
      [250_000, 7_500, 4_000, 205, 0.65, 0.50],
      [190_000, 9_500, 2_600, 205, 0.65, 0.50],
  ...
  it("安全処理3順+速攻3順を各1000戦", () => {
    const rows = [
      ...TOWER90_SAFE_FOCUS.map((focus, i) => ({ type: "SAFE", focus, scenario: TOWER90_SAFE_V7, seed: 20311190 + i * 10_000 })),
      ...TOWER90_RUSH_FOCUS.map((focus, i) => ({ type: "RUSH", focus, scenario: TOWER90_RUSH_V7, seed: 20321190 + i * 10_000 })),
      ...
      return {
        ...
        fangRageTurns: avg("戦鼓晶死亡後の狂牙獣行動"),
        bossHpRatio: avg("ボス残HP割合"),
      };
    });
    console.log("TOWER90_V7_RESULTS=" + JSON.stringify(rows));
