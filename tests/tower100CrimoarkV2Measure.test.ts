...
const S1: Skill = {
  id: "crimoark_v2_s1", name: "クリエイト・ブレイク", description: "100F V2", target: "SINGLE_ENEMY", cooldownTurns: 0,
...
const S2: Skill = {
  id: "crimoark_v2_s2", name: "リライト・ディザスター", description: "100F V2", target: "ALL_ENEMIES", cooldownTurns: 3,
  effects: [
...
const S3: Skill = {
  id: "crimoark_v2_s3", name: "クリエイト・コピー", description: "分身生成", target: "SELF", cooldownTurns: 5,
  effects: [{ kind: "GAUGE", amount: 0, applyTo: "SELF" }],
};

const cloneS1: Skill = {
  id: "crimoark_v2_clone_s1", name: "模造斬", description: "分身", target: "SINGLE_ENEMY", cooldownTurns: 0,
  effects: [{ kind: "DAMAGE", multiplier: 1.10 }, { kind: "GAUGE", amount: -0.15 }],
};
const cloneS2: Skill = {
  id: "crimoark_v2_clone_s2", name: "模造災波", description: "分身", target: "ALL_ENEMIES", cooldownTurns: 3,
  effects: [{ kind: "DAMAGE", multiplier: 0.70 }, { kind: "DEBUFF", stat: "def", amount: 0.50, durationTurns: 2, chance: 0.50 }],
};
const cloneS3: Skill = {
  id: "crimoark_v2_clone_s3", name: "模造処刑", description: "分身", target: "SINGLE_ENEMY", cooldownTurns: 4,
  targetPriority: "LOWEST_HP",
  effects: [{ kind: "DAMAGE", multiplier: 1.60, targetHpBonus: [{ hpRatio: 0.5, bonus: 0.40 }] }, { kind: "GAUGE", amount: 0.15, applyTo: "ALLIES" }],
};

// V2: V1からHP/ATK/DEF、分身火力、分身軽減、S4、40/20%火力を弱体化。
const BASE_STATS = { hp: 450_000, atk: 9_800, def: 4_600, spd: 215, criRate: 0.30, criDmg: 1.80, accuracy: 0.75, resistance: 0.60 };

function bossDef(): MonsterDefinition {
  const base = findMonster("nemesis", "DARK")!;
  return { ...base, id: "crimoark_v2", name: "クリモアーク", stats: { ...base.stats, ...BASE_STATS }, skills: [S1, S2, S3], victoryTarget: true };
}
function cloneDef(index: number): MonsterDefinition {
  const base = findMonster("nemesis", "DARK")!;
  return {
    ...base,
    id: `crimoark_v2_clone_${index}`,
    name: `クリモアークの分身${index}`,
    stats: { ...base.stats, hp: 110_000, atk: 7_500, def: 3_750, spd: 215, criRate: 0.30, criDmg: 1.80, accuracy: 0.65, resistance: 0.40 },
    skills: [cloneS1, cloneS2, cloneS3],
    victoryTarget: false,
  };
}
...
function runOne(specs: AllySpec[], seed: number, mode: Mode, grade: GearGrade) {
  ...
  const syncBoss = () => {
    ...
    if (r <= 0.40) { atk += 1_500; spd += 25; criRate += 0.20; criDmg += 0.30; factor = 1.15; }
    if (r <= 0.20) { atk += 2_000; spd += 40; criRate += 0.20; criDmg += 0.50; factor = 1.30; }
    boss.flatStatBonus = { ...boss.flatStatBonus, atk, spd, criRate, criDmg };

    const aliveClones = clones.filter((c) => c.alive).length;
    boss.mitigateAmount = aliveClones * 0.10;
  ...
  const spawnOrRefresh = () => {
    ...
    if (alive.length < maxClones && inactive) {
      inactive.alive = true;
      inactive.currentHp = inactive.maxHp;
      inactive.gauge = 0;
      inactive.cooldowns = [0, 0, 0];
      spawned += 1;
      seenDead.delete(inactive.instanceId);
    } else {
      for (const c of alive) {
        c.currentHp = Math.min(c.maxHp, c.currentHp + Math.round(c.maxHp * 0.30));
        c.gauge += 30;
      }
    }
  };
  ...
  while (!engine.getWinner() && turns < 350) {
    ...
    if (actor === boss) {
      ...
      if (s4Cd <= 0) {
        const aliveClones = clones.filter((c) => c.alive).length;
        const r = boss.currentHp / boss.maxHp;
        const factor = r <= 0.20 ? 1.30 : r <= 0.40 ? 1.15 : 1;
        const s4: Skill = {
          id: "crimoark_v2_s4",
          name: "オーバークリエイト",
          description: "100F V2",
          ...
          effects: [
            { kind: "STRIP", chance: 1 },
            { kind: "DAMAGE", multiplier: 1.30 * factor * (1 + aliveClones * 0.15) },
        ...
        const old = boss.def.skills[2];
        const oldCd = boss.cooldowns[2];
        boss.def.skills[2] = s4;
        boss.cooldowns[2] = 0;
        engine.resolveTurn(actor, { skillIndex: 2 });
        boss.def.skills[2] = old;
        boss.cooldowns[2] = oldCd;
        s4Cd = 6;
        s4Uses += 1;
...
function measure(name: string, specs: AllySpec[], mode: Mode, grade: GearGrade, seedBase: number, runs = 1000) {
  ...
  for (let i = 0; i < runs; i++) {
    ...
    else if (r.winner === "ENEMY") losses += 1;
    else draws += 1;
    turns += r.turns;
    survivors += r.survivors;
    bossHp += r.bossHpRatio;
    spawned += r.spawned;
    cloneDeaths += r.cloneDeaths;
    s4Uses += r.s4Uses;
  }
  ...
  return {
    ...
    drawRate: draws / runs,
    avgTurns: turns / runs,
    avgSurvivors: survivors / runs,
    avgBossHpRatio: bossHp / runs,
    avgClonesSpawned: spawned / runs,
    avgCloneDeaths: cloneDeaths / runs,
    avgS4Uses: s4Uses / runs,
  };
}

describe("100階クリモアークV2 一時測定", () => {
  it("TYPICAL/STRONG/FINISHED × 3攻略型を各1000戦測る", () => {
    const grades: GearGrade[] = ["TYPICAL", "STRONG", "FINISHED"];
    const rows = grades.flatMap((grade, gi) => [
      measure("分身処理型", SAFE, "CLONES", grade, 510_000 + gi * 10_000),
      measure("ボス集中型", RUSH, "BOSS", grade, 520_000 + gi * 10_000),
      measure("耐久処理型", SUSTAIN, "SUSTAIN", grade, 530_000 + gi * 10_000),
    ]);
    console.log(`TOWER100_CRIMOARK_V2_RESULTS=${JSON.stringify(rows)}`);
