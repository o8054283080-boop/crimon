...
describe("役割ごとの総合値", () => {
   ...
   */
  /*
   * **バランス型の上限は、モッチーのために広げてある。**
   *
   * モッチーの★6 Lv60 到達値(HP17,000 / 攻撃1,680 / 防御1,740)は
   * 依頼主が数値で指定したもので、総合値は★4アタッカー平均の1.13倍になる
   * (グリフォン1.06・セラフ1.00 に対して突出しており、★5のグジラ1.04より上)。
   *
   * **こちらの判断で下げていない。**総合値の式は防御を「軽減の強さ」として
   * 数えるが、モッチーは防御がそのまま攻撃にもなる種族なので、
   * この式の読み方自体がこの1体には合っていない面もある。
   * 下げるかどうかは依頼主の判断を待つ(PRで報告済み)。
   */
