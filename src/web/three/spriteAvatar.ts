 ...
 *
 * **`battleStage.ts` の RUNG / LANE_* と対で決まる。**
 * 隊列を片側2列の千鳥にして盤面の深さが半分になり、そのぶんカメラが寄った。
 * 倍率を据え置くとゴーレムが画面の横幅の4割を占め、
 * 隣の列と敵の列に食い込む。**寄ったぶんだけ倍率を下げる。**
 * いまは背丈が画面の縦の約11%、ゴーレムの横幅が約29%。
 * 参考画面は横に広い画面のもので、**画面の横幅に対する比では
 * こちらの方が既に大きい**(参考は12%)。縦持ちの390pxでは
 * これ以上大きくすると、内側の列どうしが画面の真ん中でぶつかる。
 *
 ...
 *
 * **盤面の枠を決める側(`battleStage.ts`)がこれを使う。**
 * 板をカメラの角度へ倒すと頭が奥へ動くので、その分の余白が要る。
 * 固定の数で持っていた時、表示倍率を変えたら余白だけが取り残されて
 * 盤面が画面の上へ押し上げられ、下に床だけの帯ができた。
 */
export const SPRITE_MAX_HEIGHT = 2.95 * SPRITE_SCALE;
