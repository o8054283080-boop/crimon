...
export class CreatureRig {
  ...
  floats = false;
  /**
   * 立ち姿の追加ヨー(ラジアン)。MonsterAvatar.faceToward が、相手を向く角度へ
   * これを足す(手前チームと奥チームで符号が反転する)。
   *
   * カメラは手前チームの真後ろ寄りに居るので、相手と真正面から向き合わせると
   * 手前の4体は背中しか見えない。顔・胸・翼の付け根は全部前側にあるので、
   * ここで半身に構えさせないと、造形を作り込んでも画面に出てこない。
   *
   * 目安:
   *   0.30〜0.38 … 人型・浮遊体。正面の情報量が多いので浅くてよい
   *   0.52〜0.62 … 四足・鳥獣。前後に長く、真後ろからでは団子になる
   * 大きくするほど「相手を向いていない」ように見えるので 0.62 を上限とする。
   */
  yawBias = 0.34;
  /**
