...
function renderList(props: EquipmentDungeonProps): HTMLElement {
  ...
  return el("div", { className: "screen stages-screen" }, [
    ...
    renderDungeonIntro(
      ...
      /*
       * 上の階ほど良くなることだけを、**札の短さのまま**伝える。
       * 11・12階は★4以下が出ないので「★6まで」ではなく「★5・★6のみ」。
       * 「まで」と「のみ」の一字違いで、下限が上がったことが読める。
       * **確率の数字は出さない**(依頼主との約束)。
       * 札は横に折り返すので、1枚増えても縦画面が圧迫されない。
       */
      ["1〜3階 ★4まで", "4〜6階 ★5まで", "7〜10階 ★6まで", "11〜12階 ★5・★6のみ"],
