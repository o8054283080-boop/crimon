 ...
 */
export function applyDefense(baseDamage: number, effectiveAtk: number, effectiveDef: number, ignoreDefense = false): DefenseBreakdown {
  const base = Math.max(0, baseDamage);
  if (ignoreDefense) return { afterRatio: base, flatReduction: 0, afterDefense: base };
  if (balanceFlags.defenseFormula === "sw") return applyDefenseSw(base, effectiveDef);
 ...
 * 攻撃力を受け取らないのは、**見ないから**。
 */
export function applyDefenseSw(baseDamage: number, effectiveDef: number): DefenseBreakdown {
  const base = Math.max(0, baseDamage);
 ...
 */
export function applyDefenseLegacyE(baseDamage: number, effectiveAtk: number, effectiveDef: number, ignoreDefense = false): DefenseBreakdown {
  const base = Math.max(0, baseDamage);
  if (ignoreDefense) return { afterRatio: base, flatReduction: 0, afterDefense: base };
