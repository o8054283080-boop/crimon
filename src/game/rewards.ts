...
export interface ExpAwardInfo {
  instanceId: string;
  name: string;
  /** リザルト表示時点（EXP反映後）の現在レベル */
  level: number;
  maxLevel: number;
  /** この戦闘で実際に入ったEXP。MAXメンバーは0 */
  total: number;
...
export interface ClearRewardResult {
  ...
  /** パーティ全員をスロット順で保持する。MAXメンバーもtotal=0で含む */
...
export function applyExpAndLevelUps(partyInstances: MonsterInstance[], expTotal: number): { levelUps: LevelUpInfo[]; expAwards: ExpAwardInfo[] } {
  ...
  const awardedById = new Map<string, { total: number; maxMemberBonus: number }>();
  ...
  if (trainees.length > 0) {
    ...
    for (const instance of trainees) {
      ...
      awardedById.set(instance.id, { total: awarded, maxMemberBonus: bonus });
      if (gained > 0) {
  ...
  // UIで「現在のパーティLv」を必ず4枠分確認できるよう、MAXメンバーも含めて返す。
  // EXP分配そのものは上のtraineesだけに行うため、従来の育成計算は変わらない。
  const expAwards = partyInstances.map((instance) => {
    const dex = findMonsterById(instance.dexId);
    const awarded = awardedById.get(instance.id);
    return {
      instanceId: instance.id,
      name: dex ? dex.name : instance.dexId,
      level: instance.level,
      maxLevel: STAR_MAX_LEVEL[instance.star],
      total: awarded?.total ?? 0,
      maxMemberBonus: awarded?.maxMemberBonus ?? 0,
    };
  });
