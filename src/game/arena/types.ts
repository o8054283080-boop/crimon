...
export interface ArenaDefenseSnapshot {
  ...
  /**
   * 攻撃・防御へ掛ける倍率。**レート3000より上のNPCだけが持つ。**
   *
   * ## なぜ育成の外に出たのか
   *
   * 3000あたりで、育成の範囲内で作れる強さの天井に届いた。星もレベルも装備の
   * 強化も能力ポイントも上限に張り付き、最後に残っていた装備の厳選も
   * 22本でほぼ最適に届くので、320本に増やしても差が出ない。
   *
   * それどころか**厳選は途中から逆効果になっていた。**体力型は
   * サブOPの希望がHP寄りなので、選び直すほどHPに偏り、
   * 同じ編成で 3000 と 3500 を比べると攻撃が-12%・防御が-13%・クリ率が-6pt
   * 下がっていた(HPは+23%)。噛み合いを増やしたつもりが、総合力を削っていた。
   *
   * ## HPと速度には掛けない
   *
   * 速度は手番の数に直結する。ここへ倍率を掛けると、相手だけが何度も動く
   * 別のゲームになる。**HPも掛けない**(依頼主「HPが高すぎてしまうので、HPだけは
   * 倍率をつけない」。前は掛けていて、倒し切るまで長引くだけの戦いになっていた)。
   * 掛けるのは攻撃・防御の2つだけ。
   *
   * ## プレイヤーの防衛編成は持たない
   *
   * 省略時は倍率なし。**実プレイヤーの編成にこれが付くことは無い。**
   * 付けた瞬間、防衛が「自分の育成より強い自分」になる。
   */
