...
export interface Equipment {
   ...
   * おまかせ装備の候補から外す印。
   *
   * **売却防止の `locked` とは別物。**あちらは「手放したくない」、
   * こちらは「自動で着け替える対象に入れたくない」。
   * 大事な装備に鍵をかけている人は多いので、鍵を「おまかせ対象外」と
   * 同じ意味にしてしまうと、**鍵をかけた強い装備がおまかせに一切出なくなる。**
   * 旧セーブには無いので、未定義は「対象に入れる」として読む。
   */
  autoExclude?: boolean;
  slot: EquipSlot;
  star: EquipStar;
 ...
 */
export const ENHANCE_COST_TABLE: Record<EquipStar, readonly number[]> = {
...
export function enhanceEquipmentTotalCost(star: EquipStar, from = 0, to = EQUIP_MAX_LEVEL): number {
  const table = ENHANCE_COST_TABLE[star];
  let sum = 0;
  for (let level = from + 1; level <= to; level += 1) sum += table[level] ?? 0;
  return sum;
}
