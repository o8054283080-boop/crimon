...
export interface BossTraits {
   ...
   * 「防御を20%無視するようになる」「受けるダメージが15%減る」は
   * 実数では書けない。目覚の深域の才能晶が使う。
   */
  empowerBossOnDeathRatio?: {
    /** 本体の部分防御無視率へ足す */
    defenseIgnoreRatio?: number;
    /** 本体の被ダメージ倍率へ掛ける(0.85 で15%軽減) */
    damageTakenMultiplier?: number;
  };
  /**
   * 才能適応。**同じ相手から連続で攻撃を受けるほど、
   * その相手から受けるダメージが段階的に減る。**
   *
   * 別の味方から攻撃を受けると1段階戻る。
   *
   * ## なぜこれが要るのか
   *
   * 数字を上げるだけの難易度は「もっと強い1体を作れ」で答えが出てしまう。
   * これは**1体に全部を任せる編成にだけ**代償があり、
   * 手を配れば元のままという形の要求で、
   * 「スキルがモンスターにいろんな場所での役割を与える」に沿う
   * (docs/design-concept.md)。毒も耐久も潰さない——
   * どの戦い方でも「同じ手を繰り返すか、配るか」だけを問う。
   */
  talentAdaptation?: {
    /** 1段ごとの軽減率 */
    perStack: number;
