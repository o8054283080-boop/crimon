 ...
 * 数値だけでなく、巨人ダンジョン式の仕掛け(反撃・継続ダメージ耐性・バフ剥がし・回復阻害)を
 ...
 *   バフ剥がし・回復阻害・反撃はどれも、強化と回復を積み重ねる編成(=通常モンスターの
 *   本命の勝ち筋)に刺さり、火力で押し切る高レア編成にはほとんど効かなかった
 * - **通常モンスターだけの編成は7階までが現実的な範囲**(★6Lv60+★6装備サブ4で38%)。
 *   8階以降は高レアを軸にした編成向けの関門になっている。
 *   docs/design-concept.md の方針とはぶつかるので、ここを戻すなら倍率ではなく
 *   通常モンスターとSR/SSRのベース性能差そのものを詰める方が筋が良い
 */
