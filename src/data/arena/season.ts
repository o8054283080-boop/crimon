 ...
 *
 * **必ず過去の月曜にすること。** 未来にすると週の通し番号が負になり、
 * シーズンが0のまま進まない。実際に一度そうなっている——
 * 「過去にしておくこと」と注記した直後に未来の日付を書き、
 * 実機で**「シーズン0」「今週のランク報酬は受け取り済み」**が出た
 * (未受取の目印 -1 と、負の週番号が衝突していた)。
 */
export const ARENA_SEASON_EPOCH_UTC = Date.UTC(2026, 7, 30, 19, 0, 0); // 2026-08-31 04:00 JST(月曜)

/**
 * まだ受け取っていないことを表す目印。
 *
 * **実在しうる週番号と衝突しない値にする。** -1 を使っていたため、
 * 週番号が -1 になる時期に「受け取り済み」と判定された。
 */
export const ARENA_NOT_CLAIMED = -999_999;
