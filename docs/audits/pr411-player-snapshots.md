...
9. 本番ログイン後にservice_roleのアリーナ読取権限不足（42501）を確認。関連7表へSELECTだけを付与し、既存行の更新権限は追加しない。
10. HOME画像CIが本番設定でビルドされ匿名認証を起動していた。検査・画像CIのSupabase接続を無効化。
11. snapshot一覧を検索対象にし、0件・取得失敗・未対応APIを区別できる表示を追加。
