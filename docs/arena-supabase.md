...
### 作り直した照合表は、**本番へ流すまで効かない**

`npm run arena:catalog` が作るのは `supabase/migrations/` の**新しいファイル**だけ。
CI も、そのファイルが手元の定義と揃っているかしか見ていない。
**本番の Supabase へ流す作業は、いまも手動。**
`.github/workflows/` に migration を適用する仕組みは無い。

流し忘れた時に何が起きるかは、**装備の数値を触ったか、種類を足したかで別物**:

| 触ったもの | 流し忘れた時に困る人 |
|---|---|
| モンスター・装備シリーズ・潜在を**足した** | その新しいものを使う人だけ |
| 装備の**数値**(メイン・サブの上限が動く) | **既にその装備を持っている全員** |

数値の方がはるかに広い。`arena_catalog_stat_caps` の `main_max` / `sub_max` は
装備生成の式そのものから測っているので、**係数を1つ動かすだけで表が動く。**
既存装備の移行を入れた場合はなおさらで、その日から手持ちが一斉に古い上限を超える。

超えると `arena__validate_snapshot` が `MAIN_STAT_OVER_CAP` で弾き、
**防衛の登録も、対戦の発行も落ちる。**

実際に踏んだ: メインのクリダメを1.35倍にした時、★6+15 の上限は
0.948 → 1.274 へ動いた。本番の表が古いままだと、クリダメメインの★6装備を
着けている人は**全員が対戦を発行できない**。
そして発行に失敗した挑戦は、画面には**「挑戦券が減らない」**として現れる
(手元だけで券を引き、次にサーバの残高を読んだ時に戻るため)。
いまは断られたら止めて理由を出すので、
...
手元の一番新しい `arena_catalog_*.sql` の同じ行と突き合わせる。
