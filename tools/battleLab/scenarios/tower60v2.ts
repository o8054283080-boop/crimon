 ...
 *   ・攻撃力 6,200 → 7,200     …… 一撃を重くする
 *   ・S3倍率 2.5 → 3.5         …… 同上
 *   ・反撃を S2 → S3、6回 → 5回 …… 手数への代償を、渦ではなく一閃で返す
 *   ・呪晶のS3を**回復不能**へ  …… 支え役が落ちない一番の理由へ直接触る
 *
 * **取り巻き自身のステータスは1つも触っていない。**
 *
 * ## まだ入っていないもの
 *
 * 「取り巻きが倒れると本体が伸びる」(呪晶→速度+85 / 魔晶→攻撃+1,600)は、
 * **本編エンジン側の `empowerBossOnDeath` が入ってから足す。**
 * この道具だけを先に入れられるよう、いまは依存を持たせていない。
 *
 * **そのため、この形はまだ釣り合っていない。**最良手(既存AIまかせ)で93.5%あり、
 * 期待範囲60〜85%を大きく上回る。走らせると警告が出るが、それで正しい——
 * 難易度を作っている一番の札がまだ入っていない、という意味だから。
 * 範囲の方を広げて黙らせないこと。
 *
  ...
  enemies: [
    {
      ...V1_BOSS,
      label: "古代の豪魔人",
      stats: { ...V1_BOSS.stats, hp: 150_000, atk: 7_200 },
      skills: BOSS_SKILLS,
      bossTraits: {
    ...
    },
    {
      ...
      label: "古代の魔晶",
      /*
       * ここへ「倒すと本体の攻撃力+1,600」を足す予定。
       * **本編エンジンの `empowerBossOnDeath` が入ってから。**
       * この道具だけを先に入れられるよう、依存を持たせていない。
       */
    },
    {
      ...V1_JUSHOU,
      ...
      // ここへ「倒すと本体の速度+85」を足す予定(魔晶と同じく、エンジン側が入ってから)
    },
  ],
  /*
   * 狙う順。**1つ目が既定で、そこは「実測でいちばん強い線」を置く。**
   *
   * v1 から借りたままだと既定が「呪晶→魔晶→豪魔人」(54.2%)で、
   * これは4通りで**いちばん弱い**線だった。それを基準に数値を詰めると、
   * 最悪手での勝率を見ながら調整することになる。
   * プレイヤーは最良手を見つけるので、基準もそこへ合わせる。
   *
   * **最良手は階ごとに違う。作った後に測って並べ直すこと。**
   * この形(取り込み無し)では「既存AIまかせ」がいちばん強かった(93.5%)。
   */
  focusPatterns: [
    { name: "既存AIまかせ", order: [] },
