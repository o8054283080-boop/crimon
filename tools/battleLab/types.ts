...
export interface Scenario {
  ...
  expect?: ExpectRange;
  /**
   * 試練の塔の階番号。**本編の階固有の仕掛けを、本編の実装のまま動かすための口。**
   *
   * 70階の超再生、90階の狂化・脆弱・戦鼓晶の追加テンポは、どれも
   * `src/battle/engine.ts` が `trialTowerFloor` を見て動かしている。
   * ここに階番号を書いた盤面は、**フックを1行も使わずに本編そのものを測る。**
   * 仮案を測る盤面(`hook` を持つもの)には書かない。
   */
