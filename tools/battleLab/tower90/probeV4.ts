...
/** 戦鼓晶S3時、ボスだけ追加でゲージ+30% / 全CT-1。手番を消費せず本編スキル処理を通す。 */
const bossTempoSkill: Skill = {
  id: "tower90_v4_boss_tempo",
  name: "血戦共鳴・主核加速",
  description: "ボス追加ゲージ30%・CT1短縮",
  target: "SELF",
  cooldownTurns: 0,
  effects: [
...
export function tower90ProbeV4(context: Context): ScenarioProbe {
  let turns = 0;
  let reached70 = false;
  let reached40 = false;
  let reached20 = false;
  ...
  const syncBoss = (): void => {
    ...
    let damageFactor = 1;
    if (ratio <= 0.70) {
      hpAtk += TOWER90_V4_RULES.hp70Atk;
      hpSpd += TOWER90_V4_RULES.hp70Spd;
    }
    if (ratio <= 0.40) {
      hpAtk += TOWER90_V4_RULES.hp40AtkExtra;
      hpSpd += TOWER90_V4_RULES.hp40SpdExtra;
      damageFactor = TOWER90_V4_RULES.hp40DamageFactor;
    }
    if (ratio <= 0.20) {
      hpAtk += TOWER90_V4_RULES.hp20AtkExtra;
      hpSpd += TOWER90_V4_RULES.hp20SpdExtra;
      damageFactor = TOWER90_V4_RULES.hp20DamageFactor;
    }
    const flat = unit.flatStatBonus as {
      atk?: number;
      spd?: number;
      criRate?: number;
      criDmg?: number;
    };
    flat.atk = hpAtk + kills * TOWER90_V4_RULES.escortAtk;
    flat.spd = hpSpd + kills * TOWER90_V4_RULES.escortSpd;
    flat.criRate = kills * TOWER90_V4_RULES.escortCriRate;
    flat.criDmg = kills * TOWER90_V4_RULES.escortCriDmg;
  ...
  return {
    ...
    finish() {
      ...
      return {
        ...
        "倒したお供の数": kills,
        "お供死亡狂化ATK": kills * TOWER90_V4_RULES.escortAtk,
        "お供死亡狂化SPD": kills * TOWER90_V4_RULES.escortSpd,
        "お供死亡狂化クリ率": kills * TOWER90_V4_RULES.escortCriRate,
        "お供死亡狂化クリダメ": kills * TOWER90_V4_RULES.escortCriDmg,
        "ボス行動回数": bossActions,
