...
export function tower90Probe(context: Context): ScenarioProbe {
  let turns = 0;
  let reached70 = false;
  let reached40 = false;
  let reached20 = false;
  ...
  const syncBoss = (): void => {
    ...
    let damageFactor = 1;
    if (ratio <= 0.70) {
      hpAtk += TOWER90_RULES.hp70Atk;
      hpSpd += TOWER90_RULES.hp70Spd;
    }
    if (ratio <= 0.40) {
      hpAtk += TOWER90_RULES.hp40AtkExtra;
      hpSpd += TOWER90_RULES.hp40SpdExtra;
      damageFactor = TOWER90_RULES.hp40DamageFactor;
    }
    if (ratio <= 0.20) {
      hpAtk += TOWER90_RULES.hp20AtkExtra;
      hpSpd += TOWER90_RULES.hp20SpdExtra;
      damageFactor = TOWER90_RULES.hp20DamageFactor;
    }

    unit.flatStatBonus.atk = hpAtk + kills * TOWER90_RULES.escortAtk;
    unit.flatStatBonus.spd = hpSpd + kills * TOWER90_RULES.escortSpd;
  ...
  const observeEscortDeaths = (): void => {
    ...
    previousAlive = new Set(ESCORTS.filter((id) => context.aliveOf(id)));
  };
  ...
  return {
    ...
    finish() {
      ...
      return {
        ...
        "倒したお供の数": kills,
        "お供死亡狂化ATK": kills * TOWER90_RULES.escortAtk,
        "お供死亡狂化SPD": kills * TOWER90_RULES.escortSpd,
        "ボス行動回数": bossActions,
