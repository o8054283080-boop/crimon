...
export function tower90ProbeV6(context: Context): ScenarioProbe {
  let turns = 0;
  let reached70 = false;
  let reached40 = false;
  let reached20 = false;
  ...
  const syncBoss = (): void => {
    ...
    if (ratio <= 0.40) {
      hpAtk += TOWER90_V6_RULES.hp40AtkExtra;
      hpSpd += TOWER90_V6_RULES.hp40SpdExtra;
      damageFactor = TOWER90_V6_RULES.hp40DamageFactor;
    }
    if (ratio <= 0.20) {
      hpAtk += TOWER90_V6_RULES.hp20AtkExtra;
      hpSpd += TOWER90_V6_RULES.hp20SpdExtra;
      damageFactor = TOWER90_V6_RULES.hp20DamageFactor;
    }
    const flat = unit.flatStatBonus as { atk?: number; spd?: number; criRate?: number; criDmg?: number };
    flat.atk = hpAtk + kills * TOWER90_V6_RULES.escortAtk;
    flat.spd = hpSpd + kills * TOWER90_V6_RULES.escortSpd;
    flat.criRate = kills * TOWER90_V6_RULES.escortCriRate;
    flat.criDmg = kills * TOWER90_V6_RULES.escortCriDmg;
  ...
  return {
    ...
    finish() {
      ...
      return {
        ...
        "倒したお供の数": kills,
        "お供死亡狂化ATK": kills * TOWER90_V6_RULES.escortAtk,
        "お供死亡狂化SPD": kills * TOWER90_V6_RULES.escortSpd,
        "お供死亡狂化クリ率": kills * TOWER90_V6_RULES.escortCriRate,
        "お供死亡狂化クリダメ": kills * TOWER90_V6_RULES.escortCriDmg,
        "ボス行動回数": bossActions,
