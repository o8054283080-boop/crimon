...
const TEAMS: Record<string, { ids: string[]; tuned: boolean }> = {
  ...
  "毒重ね": { ids: ["imp_GRASS", "imp_WATER", "slime_ELECTRIC", "imp_FIRE", "fairy_WATER"], tuned: false },
  "耐久": { ids: ["golem_WATER", "treant_GRASS", "fairy_WATER", "wisp_ELECTRIC", "golem_FIRE"], tuned: false },
};
...
export function measurePressure(ids: string[], floorNum: number, tuned: boolean, trials = 50): PressureResult {
  ...
  return { rate: wins / trials, enemyHpLeft: hpLeftSum / trials, actions: actions[Math.floor(trials / 2)] };
}
...
if (process.argv[1]?.endsWith("dungeonPressure.ts")) {
  ...
  for (const f of floors.length > 0 ? floors : [7, 8, 9, 10]) {
    ...
    console.log("編成".padEnd(18) + "勝率".padStart(8) + "敵残HP".padStart(10) + "行動".padStart(7));
